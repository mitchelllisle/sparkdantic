...
optional_values_strategy = st.fixed_dictionaries({
    ...
    'l': st.one_of(st.decimals(min_value=0, max_value=1000, allow_nan=False, allow_infinity=False), st.none()),
...
def test_optional_values(data):
    expected_schema = StructType([StructField('g', IntegerType(), True), StructField('h', DoubleType(), True), StructField('i', StringType(), True), StructField('j', BooleanType(), True), StructField('k', BinaryType(), True), StructField('l', DecimalType(10,0), True), StructField('z', DateType(), True), StructField('dd', TimestampType(), True), StructField('hh', DayTimeIntervalType(0, 3), True)])
